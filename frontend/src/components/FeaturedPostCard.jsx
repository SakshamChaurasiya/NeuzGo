  ...
  return (
    <article className="group relative overflow-hidden rounded-2xl border border-charcoal-100 bg-white hover:shadow-lg transition-all duration-300">
      <div className="grid grid-cols-1 lg:grid-cols-12">
        ...
        <div className="flex flex-col justify-between p-6 sm:p-8 lg:p-12 lg:col-span-5 bg-charcoal-900 text-white">
          <div className="space-y-4 sm:space-y-6">
            ...
            <div className="space-y-3">
              <Link to={`/blogs/${blog._id}`} className="block group-hover:text-indigo-300 transition-colors">
                <h3 className="font-serif text-2xl sm:text-3xl xl:text-4xl font-black leading-tight tracking-tight">
                  {blog.title}
                </h3>
              </Link>
              <p className="text-sm sm:text-base text-charcoal-300 line-clamp-4 leading-relaxed font-normal">
                {blog.description}
              </p>
            </div>

            {/* Author / Date Meta */}
            <div className="flex flex-wrap items-center gap-4 text-xs text-charcoal-400 font-semibold tracking-wide uppercase pt-2 border-t border-charcoal-800">
          ...
          <div className="pt-6 sm:pt-8 lg:pt-0">
            <Link
              to={`/blogs/${blog._id}`}
              className="inline-flex items-center justify-center px-6 py-3 text-sm font-bold uppercase tracking-wider text-charcoal-900 bg-white rounded-lg hover:bg-indigo-50 transition-colors duration-300"
            >
              Read Article
            </Link>
          </div>
